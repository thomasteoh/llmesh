// llmesh landing — routing visualisation
(() => {
  const el = document.getElementById("route-canvas") as HTMLCanvasElement | null;
  if (!el) return;
  ...
  type Kind = "client" | "router" | "worker";
  type Phase = "go" | "wait" | "fly" | "die";
  ...
  const hub: Node = { fx: 0.5, fy: 0.5, label: "llmesh", kind: "router", glow: 0 };
  ...
  const allNodes: Node[] = [...clients, hub, ...workers];
  ...
  const nx = (n: Node) => n.fx * cw;
  const ny = (n: Node) => n.fy * ch;
  ...
  function update(dt: number): void {
    ...
    packets = kept;
    for (const n of allNodes) n.glow = Math.max(0, n.glow - dt * 1.6);
  }
  ...
  function drawColumnLabels(): void {
  ...
  function drawPacket(p: Packet): void {
    if (p.phase === "wait" || p.phase === "die") return;
    let x: number;
    let y: number;
    if (p.phase === "go") {
      const t = ease(p.t);
